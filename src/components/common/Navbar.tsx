...
import { setSearchQuery } from '@/store/productsSlice';
import CartButton from '../CartButton';
import { navbarConfig, Category } from '@/types/categories';
...
export default function Navbar({ categories = [], showCategories = true }: NavbarProps) {
  ...
  const images = ['/assets/navbar/ig1.png', '/assets/navbar/ig2.png'];

  // Calculate dropdown height
  useEffect(() => {
    if (leftRef.current) setDropdownHeight(leftRef.current.offsetHeight);
  }, [hoveredLink]);
  ...
  const handleMouseLeave = () => {
    leaveTimeoutRef.current = setTimeout(() => setHoveredLink(null), 200);
  };

  // Determine current navbar config from path
  const matchedConfig =
    Object.entries(navbarConfig).find(([key]) => pathname.startsWith(key))?.[1] ??
    { categories: [] };
 const mergedCategories: Category[] = categories.length
  ? categories
  : matchedConfig.categories || [];
  ...
  return (
    <header className="w-full relative">
      ...
      <nav className="flex justify-between items-center py-4 px-6 border-b relative">
        ...
        {/* Logo */}
        <div className="font-bold text-xl">
          <Link href="/">EVERLANE</Link>
        </div>

        {/* Right icons */}
        <div className="flex items-center space-x-4">
          <Search size={20} className="cursor-pointer" onClick={() => setSearchOpen(!searchOpen)} />
          <User size={20} />
          <CartButton />
          <span className="hidden sm:inline">USD</span>
          <button className="md:hidden ml-2" onClick={() => setMenuOpen(!menuOpen)}>
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 w-full bg-white shadow-md md:hidden z-50 h-auto">
            <div className="flex flex-col p-4 space-y-3">
              ...
              <h4 className="font-semibold mr-2 mb-2">
                Subcategories <span className="text-gray-900 text-md">↓</span>
              </h4>
              <div className="relative h-40 overflow-y-auto px-2 py-4 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
      ...
      {hoveredLink && (
        <div
          className="absolute left-0 w-full bg-white z-50 px-10 py-20 border-t"
          style={{ top: '100%' }}
        ...
        >
          <div className="grid grid-cols-2 w-full gap-10">
            ...
            <div className="grid grid-cols-2 gap-4 flex-1">
              {images.map((src, idx) => (
                <div key={idx} className="relative flex-1 group" style={{ height: dropdownHeight }}>
                  <Image src={src} alt={`Look ${idx + 1}`} fill className="rounded-md object-cover" />
                  <div className="absolute inset-0 bg-black bg-opacity-30 opacity-0 group-hover:opacity-100 transition-opacity rounded-md" />
                </div>
              ))}
            </div>
        ...
        <div className="w-full bg-white/80 shadow-md px-6 py-4">
          <div className="max-w-6xl mx-auto">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                if (searchValue.trim()) router.push('/search');
              }}
              className="flex gap-2"
            >
              <input
