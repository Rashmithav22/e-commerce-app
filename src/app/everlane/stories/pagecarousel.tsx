...
export default function PageCarousel({
  ...
  const loopedProducts = [...products, ...products, ...products]; // triple for seamless loop
  const startIndex = n;
  ...
  return (
    <section className="py-2 px-1 md:px-12 relative" onMouseEnter={stopAutoplay} onMouseLeave={startAutoplay}>
...
>
  <ProductCard
    ...
    />
      ...
      {/* Arrows */}
      <button onClick={prev} className="absolute -left-6 md:-left-6 top-1/2 -translate-y-1/2 z-10">
        <ChevronLeft className="w-6 h-6 md:w-10 md:h-10 stroke-[1]" />
      </button>
      <button onClick={next} className="absolute -right-6 md:-right-6 top-1/2 -translate-y-1/2 z-10">
